...
contract Rewarder is IRewarder, Ownable {
    ...
    event LogOnReward(IERC20 indexed reward, address indexed to, uint256 amount, bool native);
    ...
    function onReward(IERC20 reward, address payable to, uint256 amount, bool native) external override onlyStaking {
        if (native) {
            require(address(this).balance >= amount, "Not enough reward");
            to.transfer(amount);
        } else {
            require(reward.balanceOf(address(this)) >= amount, "Not enough reward");
    ...
    function update(IERC20 reward, bool isNative) public onlyStaking returns (uint256) {
